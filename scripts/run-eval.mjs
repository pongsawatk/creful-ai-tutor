...
const OFF_TOPIC_ANSWER_PATTERN = /เลขเด็ด|ราคาหุ้น|ซื้อหุ้น|ขายหุ้น|สูตรอาหาร|ผลบอล|พรีเมียร์ลีก|แทงบอล|พยากรณ์อากาศ|อุณหภูมิ|นายกรัฐมนตรี|เลือกตั้ง/u;
...
function evaluateCase(testCase, response) {
  ...
  if (prefix === 'A') {
    ...
    if (OFF_TOPIC_ANSWER_PATTERN.test(answer)) {
      failures.push('D rule failed: answer contains off-topic answer keyword');
    }
  } else if (prefix === 'E') {
